...
if bge.logic.mouse.events[bge.events.LEFTMOUSE] == bge.logic.KX_INPUT_JUST_ACTIVATED:
   ...
   # Assign current clicked and visible objects as fake object to avoid 'referenced before assignment' errors
    ...
    # Open workbook with information about cities
    workbookCities = xlrd3.open_workbook(os.path.join(os.path.abspath(''), 'Cities.xls'))
    workbookCities.sheet_names()
    
    # Parse sheetCities
    sheetCities = workbookCities.sheet_by_name('Cities')
    for sheetCitiesColumn in range(sheetCities.ncols):
        if sheetCities.cell(0,sheetCitiesColumn).value == 'Date':
            sheetCitiesColumnDate = sheetCitiesColumn
        if sheetCities.cell(0,sheetCitiesColumn).value == 'Foundation':
            sheetCitiesColumnFoundation = sheetCitiesColumn
        if sheetCities.cell(0,sheetCitiesColumn).value == 'Status':
            sheetCitiesColumnStatus = sheetCitiesColumn
        if sheetCities.cell(0,sheetCitiesColumn).value == 'Date':
            sheetCitiesColumnDate = sheetCitiesColumn
        if sheetCities.cell(0,sheetCitiesColumn).value == 'Date':
            sheetCitiesColumnDate = sheetCitiesColumn
    ...
    # Open workbook with city's objects
    workbookCity = xlrd3.open_workbook(os.path.join(os.path.abspath(''), 'Cities', 'Irkutsk', 'Irkutsk.xls'))
